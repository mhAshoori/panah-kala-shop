...
<p style="color:#333;line-height:1.8">سفارش شما با کد <strong>${orderId}</strong> تحویل پست شد و در راه است.</p>
<p style="margin:24px 0"><a href="${process.env.NEXT_PUBLIC_SITE_URL ?? 'http://localhost:3000'}/order/${order.id}" style="background:#2563eb;color:#fff;text-decoration:none;padding:12px 28px;border-radius:8px;display:inline-block">پیگیری سفارش</a></p>
</div></body></html>`,
  });
}
