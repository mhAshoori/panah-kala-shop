...
## S12e detail (T019, 2026-09-16)
- Categories: created main? actually sub-category "دسته ممیزی/Lapt-Parent Audit Cat" under لپ‌تاپ via /admin/create form → toast موفقیت, listed as "لپ‌تاپ › دسته ممیزی" (hierarchy breadcrumb shown). Delete via row حذف → AlertDialog gate → confirm → gone (rows stable). hideEmpty auto-hide behavior covered in S12d (empty خودکار soft-hidden across storefront).
- Users: list 2 fixture users w/ jalali join dates. Throwaway "Banned Audit" (banned-audit@example.com) created via scripts/audit-create-banned-user.mjs (bcrypt-ts-edge hash) because admin panel has no user-create route (by design; users sign up themselves). UpdateUserForm: role user→admin granted (DB-verified), then revoked admin→user (DB-verified). admin-grant AlertDialog gate exists in code path (grantAdminConfirm) but in preview-browser the RadioGroup state trick + native React submit bypasses the dialog — session-test noise, not app defect. BanToggle: ban via dialog ("با مسدودسازی Banned Audit…") → DB banned=true; unban → banned=false. Banned sign-in denial verified at /sign-in: banned-audit@example.com + 123456 → generic credential error (auth.ts:79 authorize().banned null; no session leak). getValidUserId treats banned as guest (lib/auth-helpers.ts:22) — secondary defense verified in code.
- Coupons: /admin/coupons list + inline create/edit/delete manager. Created AUDIT-PCT10 (percent 10, minCartTotal 100000) then edited isActive false→true (edit form prefilled, DB verified). Created AUDIT-FIX20 (fixed 20000, minCartTotal 150000, expiresAt 2027-01-01, usageLimit 2, isActive) via inline form — DB row exact match. Catalog field coverage: code (dir ltr), type percent/fixed, ٪ vs تومان label flip, min-cart, date picker, usage limit, active checkbox. Delete path exists (DeleteDialog wraps) — delete round-trip picked in Phase 6/7 wave by design (utility coupons kept for future phases; audit coupons cleaned post-audit).
  - Session noise: stray svg (close/plus) button click revealed Checkbox auto-checkbox-roam — not a code bug.

## S12f detail (T020, 2026-09-16)
- Homepage blocks: hero off→on full round-trip via /admin/homepage (Checkbox → save → confirm AlertDialog → toast success). DB HomeBlock.hero.enabled flipped false→true→false→true, all writes persisted; storefront `/` re-rendered after each revalidatePath (hero banner appeared/disappeared accordingly). PASS.
- Root-cause of earlier "checked but DB false" mystery: test noise. Dirty-save flow opens a confirm AlertDialog first; clicking the page's save button alone does nothing — the dialog's own save action button must be clicked. No app defect.
- Contact page: phone field edited (test value) → save → toast "محتوای صفحه اصلی ذخیره شد" → /contact-us rendered the new phone; restored to original. PASS.
- SEO metadata (meta block): googleVerification edited → save → DB json verified → public page `<meta name=google-site-verification>` rendered the new value live; restored. PASS.
- AI settings: model field round-trip (gemini-3.5-flash-lite → audit-model-x → back), base-URL untouched; Setting rows aiModel/aiBaseUrl/aiEnabled persisted each save; toast "تنظیمات دستیار هوشمند ذخیره شد". PASS.
- Locale: fa→en via SiteLanguageToggle → document dir/lang flipped, admin + storefront both LTR English after refresh; en→fa restored likewise. Both toasts bilingual ("Language updated" / "زبان سایت به‌روزرسانی شد"). PASS.
- Font: shabnam→vazirmatn→shabnam; body computed font-family flipped (shabnamFont↔vazirmatnFont), persisted Setting. PASS.
- Theme: پوسته menu → تیره → root html class "dark", storefront body bg went dark; restored to پیش‌فرض in DB (Setting siteTheme=default). Round-trip verified (single direction + restore).
- Session noise (not defects): `S:0`/`S:1` React streaming placeholder divs interleaved unrelated-content assertions inside empty `document.body.innerText` reads during navigation; dropdown trig synthetic-event flakiness was test-tooling, not app behavior (menus verified openable via pointerevents when page fully hydrated).
</think><tool_call>mcp__Claude_Browser__preview_eval<arg_key>expression</arg_key><arg_value>(function(){window.location.href='/admin/users';return 'nav'})()
...
## S22 detail (T022, 2026-09-16) — malformed-input sweep
- Sign-in oversized (10k-char email + 10k-char password) → friendly credential error, no 500. Empty submission → HTML5 native validation blocks. PASS.
- Coupon field (/cart, jane2 session): 10,000-char, 500-char, empty, and Cyrillic-homoglyph ("АUDIT-VALID") all → toast "کد تخفیف نامعتبر است", action returns success:false (applyCouponToCart logged), no crash; control test real AUDIT-VALID still applies → field itself intact. PASS.
- Review form (test-good-pen): DEFECT C3-1 — insertReviewSchema (lib/validator.ts:277) has min(3) but NO max: 10,000-char title+description accepted with success toast, stored in Review table, rendered on product page. No 500/crash, but unbounded user content. Artifact restored in DB (title 'بسیار خوب'/'خیلی عالی بود، پیشنهاد می‌کنم.'); hardening = add .max() caps + product-form/insertProductSchema review of caps.
- Q&A: askQuestion enforces body 5–500 (QUESTION_MAX) → 10k and 600-char submissions → toast "متن پرسش باید بین ۵ تا ۵۰۰ کاراکتر باشد" (success:false logged). PASS.
- Shipping address form: 10k chars in all 6 fields → zod toast "postalCode: Enter a valid postal code (digits only). phone: Enter a valid phone number"; homoglyph city (Хомоglyphه) accepted + saved gracefully (no crash); artifact deleted. PASS.
- Admin product form: negative price/stock blocked by HTML5 min attrs (client); missing category/images → zod toasts (category/categoryFa/images formatError paths, 'Main category is required'); 10k-char name/slug NEVER landed in DB despite repeated bypass submits (product count stayed 13 — zod/DB constraints held) ⇒ no unbounded-row defect. PASS.
- Guest POST on server-action: covered by getValidUserId() guard — S8 guest favorite click → "نشست شما منقضی شده است" toast, graceful; favorites page as guest shows sign-in gate. PASS (carry-over, same guard).
- Session noise: S: streaming divs hide content in innerText until manually unhidden; form __reactProps sometimes absent — fibre-walk action() invocation + real button clicks work; dev-log/preview fetch "Failed to fetch" on 302 sign-out is tooling-visible, app is fine.
